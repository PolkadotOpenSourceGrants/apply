...
As of today, many projects and endeavors in the ecosystem try to create native libraries for other programming languages than Rust to interact with Polkadot SDK based chains, this is great, but it comes with a cost:

- Safety may be at risk: Cryptographic libraries are not interchangeable between programming languages, no one can guarantee that cryptographic outputs from each of those languages are the absolute equivalent to the Rust based cryptographic primitives used in Polkadot SDK.

- Implementations of SDKs for Polkadot SDK is subject to interpretation and programming styles, which is not a bad thing, but as for cryptography, it may be the origin for intrinsic bugs in an extreme environment such as the blockchain.

- Keeping those SDKs in par with the official ones (Polakdot SDK itself, Subxt etc) is huge task itself, there's no substitute for dedication to keep up with the upstream libraries and test rigorously.

I've learned from the classes I took in PBA Lucerne about the full potential of **Subxt** and why it was initiated (along with *Smoldot*) by Parity: the community needed a Rust based library that gives developers a robust way to interact with Polkadot SDK based chains while guaranteeing type safety and cryptographic homogeneity by using the same core cryptographic Rust primitives used in Polkadot SDK itself.
